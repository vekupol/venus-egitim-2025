...
function SayiKumeleri({ unitsData, activeDiv, setActiveDiv }) {
            ...
            {unitsData.length > 0 && (
              <ProgressBar
                donePeople={unitsData.reduce((acc, unit) => {
                  return (
                    acc +
                    unit.parts[0]?.lessons.reduce((partAcc, part) => partAcc + part.done, 0)
                  );
                }, 0)}
                totalPeople={unitsData.reduce((acc, unit) => {
                  return (
                    acc +
                    unit.parts[0]?.lessons.reduce(
